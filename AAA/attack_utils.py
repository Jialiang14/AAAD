...
def L1_target_loss(x, y, y_target):
    return x[np.arange(x.shape[0]), y_target]

def L1_loss(x, y):
    return -x[np.arange(x.shape[0]), y]

def untarget_dlr_loss(x, y):
    x_sorted, ind_sorted = x.sort(dim=1)  #对x进行在第二维度从小到大排序，并返回索引
    ind = (ind_sorted[:, -1] == y).float()
    u = torch.arange(x.shape[0])
    return -(x[u, y] - x_sorted[:, -2] * ind - x_sorted[:, -1] * (
        1. - ind)) / (x_sorted[:, -1] - x_sorted[:, -3] + 1e-12)
    #ind为0，则分类置信度最高的那一类不是标签，ind为1，则分类置信度高的那一类是标签，应该取第二大的输出
...
def target_hinge_loss(x, y, y_target):
    x_sorted, ind_sorted = x.sort(dim=1)
