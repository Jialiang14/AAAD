...
import numpy as np
import torch
import optimizer_adv.SmoothDARTS.optimizers.darts.utils as utils
import logging
import argparse
...
from optimizer_adv.SmoothDARTS.optimizers.darts.architect import Architect
...
parser.add_argument('--save', type=str, default='CAA', help='experiment name')
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, perturb_alpha, epsilon_alpha):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        input_search_noise = CAA(model, args, subpolicy, input_search, target_search)

        architect.step(input, target, input_search_noise, target_search, lr, optimizer, unrolled=args.unrolled)
        optimizer.zero_grad()
        ...
        if perturb_alpha:
            perturb_alpha(model, noise_image, target, epsilon_alpha)
            optimizer.zero_grad()
            architect.optimizer.zero_grad()
        # print('after perturb', model.arch_parameters())

        logits = model(noise_image, updateType='weight')
