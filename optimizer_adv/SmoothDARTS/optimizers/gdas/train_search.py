...
args.save = '../../experiments/gdas/search_space_{}/search-{}-{}-{}-{}-{}'.format(
    args.search_space, args.save, time.strftime("%Y%m%d-%H%M%S"), 
    args.seed, args.learning_rate, args.search_space)
if args.unrolled:
    args.save += '-unrolled'
...
def main():
    ...
    model = Network(tau_epoch, args.init_channels, CIFAR_CLASSES, args.layers, criterion,
    ...
    for epoch in range(args.epochs):
        ...
        # training
        train_acc, train_obj, ev = train(train_queue, valid_queue, model, architect, 
                                         criterion, optimizer, lr, epoch, analyzer)
        ...
        # Decrease the temperature for the gumbel softmax
        # linearly like in gdas
        tau_epoch += tau_step
