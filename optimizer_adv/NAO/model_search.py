...
from operations import WSAvgPool2d, WSMaxPool2d, WSReLUConvBN, WSBN, WSSepConv, FactorizedReduce, MaybeCalibrateSize, AuxHeadCIFAR, AuxHeadImageNet, apply_drop_path, NAOZero, WSDilConv
...
class Node(nn.Module):
    def __init__(self, prev_layers, channels, stride, drop_path_keep_prob=None, node_id=0, layer_id=0, layers=0, steps=0):
        ...
        self.x_dil_conv_3 = WSDilConv(num_possible_inputs, channels, channels, 3, None, 2, 2)
        self.x_dil_conv_5 = WSDilConv(num_possible_inputs, channels, channels, 5, None, 4, 2)
        ...
        self.y_dil_conv_3 = WSDilConv(num_possible_inputs, channels, channels, 3, None, 2, 2)
        self.y_dil_conv_5 = WSDilConv(num_possible_inputs, channels, channels, 5, None, 4, 2)
        self.skip = FactorizedReduce(channels, channels, affine=True)
    ...
    def forward(self, x, x_id, x_op, y, y_id, y_op, step, bn_train=False):
        ...
        if x_op == 0:
            ...
            x = self.zero(x, stride)
        elif x_op == 6:
            x = self.x_dil_conv_3(x, x_id, stride, bn_train=bn_train)
        elif x_op == 7:
            x = self.x_dil_conv_5(x, x_id, stride, bn_train=bn_train)
        else:
            assert x_op == 8
            if stride > 1:
                assert stride == 2
        ...
        if y_op == 0:
            ...
            y = self.zero(y, stride)
        elif y_op == 6:
            y = self.y_dil_conv_3(y, y_id, stride, bn_train=bn_train)
        elif y_op == 7:
            y= self.y_dil_conv_5(y, y_id, stride, bn_train=bn_train)
        else:
            assert y_op == 8
            if stride > 1:
                assert stride == 2
...
class NASWSNetworkCIFAR(nn.Module):
    def __init__(self, classes, layers, nodes, channels, keep_prob, drop_path_keep_prob, use_aux_head, steps):
        ...
        self.pool_layers = [self.layers // 3, 2 * self.layers // 3]
