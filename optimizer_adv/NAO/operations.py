...
class WSDilConv(nn.Module):
    def __init__(self, num_possible_inputs, C_in, C_out, kernel_size, stride, padding, dilation, affine=True):
        super(WSDilConv, self).__init__()
        self.num_possible_inputs = num_possible_inputs
        self.C_out = C_out
        self.C_in = C_in
        self.padding = padding
    ...
    def forward(self, x, x_id, stride=1, bn_train=False):
        x = self.relu1(x)
        x = F.conv2d(x, self.W1_depthwise[x_id], stride=stride, padding=self.padding, dilation=self.dilation, groups=self.C_in)
        x = F.conv2d(x, self.W1_pointwise[x_id], padding=0)
        x = self.bn1(x, x_id, bn_train=bn_train)
        return x
...
class NAOZero(nn.Module):

  def __init__(self,):
    super(NAOZero, self).__init__()
