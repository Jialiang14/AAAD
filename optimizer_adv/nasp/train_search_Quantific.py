...
parser.add_argument('--evaluation', default='Hessian', help='Jacobian | Hessian')
args = parser.parse_args()

args.save = args.evaluation
...
def train(train_queue, valid_queue, model, architect, criterion_train, optimizer, lr, epoch):
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    if not args.single_level:
      clean_loss, _, _ = criterion_train(logits, target, aux_input)
    else:
      clean_loss, loss1, loss2 = criterion_train(logits, target, aux_input)

    if args.evaluation == 'Jacobian':
      jacobian_loss = 0.01 * reg(input, logits)
      loss = clean_loss + jacobian_loss

    elif args.evaluation == 'Hessian':
      criterion = nn.CrossEntropyLoss()
      criterion = criterion.cuda()
      reg = loss_cure(model, criterion, lambda_=1, device='cuda')
      w_regularizer, grad_norm = reg.regularizer(input, target, h=0.4)
      loss = clean_loss + 10 * w_regularizer
