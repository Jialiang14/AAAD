...
import pdb
from noise.pgd import AttackPGD
...
parser.add_argument('--save', type=str, default='PGD_1_100', help='experiment name')
...
def main():
  ...
  model = Network(args.init_channels, CIFAR_CLASSES, args.layers, criterion_train, args.greedy, args.l2)
  model = model.cuda()
  ...
  for epoch in range(args.epochs):
    ...
    train_acc, train_obj, alphas_time, forward_time, backward_time = train(train_queue, valid_queue, model, architect, criterion_train, optimizer, lr)
    end_time = time.time()
...
def train(train_queue, valid_queue, model, architect, criterion_train, optimizer, lr):
  ...
  for step, (input, target) in enumerate(train_queue):
    ...
    logging.info('genotype = %s', genotype)
    logging.info('maxpool_number = %s', number)
    ...
    loss_fgsm = nn.CrossEntropyLoss()
    # noise_image = fgsm_attack(model, loss_fgsm, input, target, 0.01)
