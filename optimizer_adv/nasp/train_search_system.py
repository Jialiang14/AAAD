...
class SystemDataset(torch.utils.data.Dataset):
  def __init__(self, transform):
    images = np.load('/mnt/share1/sunjialiang/AAAD/noise/CIFAR-10-S/' + args.evaluation + '.npy')
    labels = np.load('/mnt/share1/sunjialiang/AAAD/noise/CIFAR-10-S/label_clean.npy')
    assert labels.min() >= 0
...
def train(train_queue, valid_queue, model, architect, criterion_train, optimizer, lr):
  ...
  for step, (input, soft_target) in enumerate(train_queue):
    ...
    input_search, target_search_soft = next(iter(valid_queue))
    input_search = Variable(input_search, requires_grad=False).cuda()
    target_search = target_search_soft.argmax(dim=1)
    ...
    if not args.single_level:
      loss, _, _ = criterion_train(logits, target, aux_input)
    else:
      loss, loss1, loss2 = criterion_train(logits, target, aux_input)
