...
class CNN(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.num_layers = args.child_num_layers #6
        self.out_filters = args.child_out_filters #20
        self.num_branches = args.child_num_branches #5
        self.num_cells = args.child_num_cells #5, 即cell个数
        self.use_aux_heads = args.child_use_aux_heads #False
        self.fixed_arc = None
        pool_distance = self.num_layers // 3 #2
        self.pool_layers = [pool_distance, 2*pool_distance+1] #[2,5]
    ...
    def _compile_cell(self, module, curr_cell, out_filters):
        ...
        module.add_module('dilthree', nn.ModuleList())
        self._compile_dilconv(module.dilthree, curr_cell, 3, out_filters, 2, 2)
        module.add_module('dilfive', nn.ModuleList())
        self._compile_dilconv(module.dilfive, curr_cell, 5, out_filters, 4, 2)
    ...
    def _compile_dilconv(self, module, curr_cell, filter_size, out_filters, padding, dilation):
        num_possible_inputs = curr_cell+2
        for i in range(num_possible_inputs):
            module.append(nn.Sequential(
                nn.ReLU(),
                nn.Conv2d(out_filters, out_filters, filter_size, padding=padding, dilation=dilation,
                          groups=out_filters, bias=False),
                nn.Conv2d(out_filters, out_filters, kernel_size=1, padding=0, bias=False),
    ...
    def _enas_cell(self, x, module, curr_cell, prev_cell, op_id):
        if op_id == 0:
            ...
            out = module.dilthree[prev_cell](x)
        else:
            out = module.dilfive[prev_cell](x)
        return out
