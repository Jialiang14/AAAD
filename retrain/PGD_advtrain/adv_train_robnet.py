...
parser.add_argument('--arch', type=str, default='RobNet_AAA_3', help='which architecture to use')
parser.add_argument('--grad_clip', type=float, default=5, help='gradient clipping')

args = parser.parse_args()

args.save = 'trained_model_cifar10/{}-{}'.format(args.arch, time.strftime("%Y%m%d-%H%M%S"))
...
def main():
  ...
  model = models.robnet(architecture_code.robnet_AAA_3)
  model = model.cuda()
  ...
  for epoch in range(args.epochs):
    ...
    #     best_acc = valid_acc
    utils.save(model, os.path.join(args.save, 'weights'+str(epoch)+'.pt'))
