...
def plot_2d_contour(surf_file, surf_name='train_loss', vmin=0.1, vmax=10, vlevel=0.5, show=False):
    ...
    fig = plt.figure()
    ax = Axes3D(fig)
    surf = ax.plot_surface(X, Y, Z, vmin=0, vmax=8, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    # ax.set_zlim(0,5)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--surf_file', '-f', default='/mnt/jfs/sunjialiang/AAAD/retrain/standard_train/trained_model/Random_search_PGD-20221207-211953/weights.pt_weights_xignore=biasbn_xnorm=filter_yignore=biasbn_ynorm=filter.h5_[-1.0,1.0,50]x[-1.0,1.0,50]_datasplit=1000_splitidx=0.h5', help='The h5 file that contains surface values')
    # parser.add_argument('--surf_file', '-f',
    #                     default='/mnt/jfs/sunjialiang/AAAD/retrain/standard_train/trained_model/Random_search_FGSM-20221208-012311/weights.pt_weights_xignore=biasbn_xnorm=filter_yignore=biasbn_ynorm=filter.h5_[-1.0,1.0,50]x[-1.0,1.0,50]_datasplit=1000_splitidx=0.h5',
    #                     help='The h5 file that contains surface values')
    # parser.add_argument('--surf_file', '-f',
    #                     default='/mnt/jfs/sunjialiang/AAAD/retrain/standard_train/trained_model/Random_search_Clean-20221212-095006/weights.pt_weights_xignore=biasbn_xnorm=filter_yignore=biasbn_ynorm=filter.h5_[-1.0,1.0,50]x[-1.0,1.0,50]_datasplit=1000_splitidx=0.h5',
    #                     help='The h5 file that contains surface values')
    parser.add_argument('--surf_file', '-f',
                        default='/mnt/jfs/sunjialiang/AAAD/retrain/standard_train/trained_model/ResNet18-20230404-100449/weights.pt_weights_xignore=biasbn_xnorm=filter_yignore=biasbn_ynorm=filter.h5_[-1.0,1.0,50]x[-1.0,1.0,50]_datasplit=1000_splitidx=0.h5',
                        help='The h5 file that contains surface values')
