...
if __name__ == '__main__':
    ...
    num_list1 = []
    num_list2 = []
    for corrupt_type in ['snow', 'contrast', 'brightness']:
        ...
        for i in model_list:
            ...
            logging.info('Natural Accuracy = %f', Ra_natural)
            logging.info('Robust Accuracy under Natural = %f', Ra_natural)
            if corrupt_type == 'snow':
                num_list.append(Ra_natural)
            elif corrupt_type =='contrast':
                num_list1.append(Ra_natural)
            elif corrupt_type == 'brightness':
                num_list2.append(Ra_natural)
    ...
    total_width, n = 0.7, 3
    width = total_width / n
    plt.bar(x, num_list, width=width, label="snow", fc="#0087cb")
    for i in range(len(x)):
        x[i] = x[i] + width
    plt.bar(x, num_list1, width=width, label="contrast", tick_label=name_list, fc="#ffa200")
    for i in range(len(x)):
        x[i] = x[i] + width
    plt.bar(x, num_list2, width=width, label="brightness", fc="#9966ff")
    plt.xlabel("Evaluation in search", fontsize=20)
    plt.ylabel("RA(%)", fontsize=20)
    plt.title("Evaluation after search: Natural", fontsize=20)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.legend(fontsize=14, loc='upper right')
    plt.savefig('DE_Natural.png', bbox_inches='tight', pad_inches=0.1)
