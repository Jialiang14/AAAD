...
if __name__ == '__main__':
    ...
    for eps in [1 / 255, 2 / 255, 3 / 255]:
        ...
        for i in model_list:
            ...
            Ra_adv_PGD = RA_infer(model, valid_queue, args, subpolicy)
            logging.info('Attacker %s', subpolicy)
            logging.info('Robust Accuracy under PGD = %f', Ra_adv_PGD)

            if eps == 1/255:
                num_list.append(Ra_adv_PGD)
            elif eps == 2/255:
                num_list1.append(Ra_adv_PGD)
            elif eps == 3/255:
                num_list2.append(Ra_adv_PGD)
    ...
    plt.title("Evaluation after search: PGD", fontsize=20)
    plt.xticks(fontsize=20)
    plt.yticks(fontsize=20)
    plt.legend(fontsize=14, loc='upper right')
    plt.savefig('DE_PGD.png', bbox_inches='tight', pad_inches=0.1)
